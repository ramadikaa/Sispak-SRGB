<x-app-layout>
	<x-slot name="title">Hasil Perhitungan</x-slot>

	@if(session()->has('success'))
	<x-alert type="success" message="{{ session()->get('success') }}" />
	@endif
	<x-card title="Berikut hasil Rekomendasi">
		<p class="mb-4">
		...
		<div class="mt-5">
			<div class="alert alert-success">
				...
				<p>Berdasarkan dari kebiasaan yang kamu pilih atau anak alami juga berdasarkan Role/Basis aturan yang sudah ditentukan oleh seorang pakar / guru maka perhitungan Algoritma Certainty Factor mengambil nilai CF yang paling pinggi yakni <b>{{ number_format(unserialize($riwayat->cf_max)[0], 3) }} ({{ number_format(unserialize($riwayat->cf_max)[0], 3) * 100 }}%)</b> yaitu <b>{{ unserialize($riwayat->cf_max)[1] }}</b></p>
				<p><b>Penyebab:</b> {{ $riwayat->penyakit->penyebab ?? '-' }}</p>
			</div>
			<div class="mt-3 text-center">
				...
				<a href="{{ route('admin.diagnosa') }}" class="btn btn-warning mr-1"><i class="fas fa-redo mr-1"></i> Cek ulang</a>
			</div>
		</div>
	</x-card>
</x-app-layout>